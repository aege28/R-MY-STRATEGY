...
library(caTools) #for moving window statistics, GIF, Base64, ROC, etc.
library(lubridate) #dealing with dates and times
library(scales) #graphical scales mapping data to aesthetics
library(ggplot2) #data visualization
library(RColorBrewer) #color palettes for maps and other graphics

# install.packages("dplyr") - Data manipulation function
library(dplyr) # for if_else()

# install.packages("lattice") - data visualization using the grid graphics system
...
#calculate the Calmar Ratio of a returns series 'x'.
...
# mySR function - calculates the Sharpe Ratio of a returns series 'x'.
...
for (selected_quarter in c("2021_Q1", "2021_Q3", "2021_Q4", 
  ...
  # lets calculate EMA65 and EMA115 for NQ
  data.group1$NQ_EMA65 <- EMA(na.locf(data.group1$NQ), 65)
  data.group1$NQ_EMA115 <- EMA(na.locf(data.group1$NQ), 115)
  
  # and EMA14 and EMA175 for SP
  data.group1$SP_EMA14 <- EMA(na.locf(data.group1$SP), 14)
  data.group1$SP_EMA175 <- EMA(na.locf(data.group1$SP), 175)
  
  
  # put missing value whenever the original price is missing
  data.group1$NQ_EMA65[is.na(data.group1$NQ)] <- NA
  data.group1$NQ_EMA115[is.na(data.group1$NQ)] <- NA
  data.group1$SP_EMA14[is.na(data.group1$SP)] <- NA
  data.group1$SP_EMA175[is.na(data.group1$SP)] <- NA
  ...
  data.group1$positionNQ.mom <- ifelse(lag.xts(data.group1$NQ_EMA65) >
                                         lag.xts(data.group1$NQ_EMA115),
                                       1, -1)
  
  data.group1$positionSP.mom <- ifelse(lag.xts(data.group1$SP_EMA14) >
                                         lag.xts(data.group1$SP_EMA175),
                                       1, -1)
  ...
  data.group1$positionSP.mom[times(times_) <= times("10:00:00") | 
